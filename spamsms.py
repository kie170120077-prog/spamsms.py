...
def check_internet():
    fig = Figlet(font="big")
    banner = fig.renderText("TK Tool")
    console.print(Align.center(f"[bold cyan]{banner}[/bold cyan]"))
    console.print(Align.center("🌐 Kiểm Tra Kết Nối Mạng 🌐", style="cyan"))
    ...
    try:
        ...
        console.print(Align.center("[bold green]✅ Đã kết nối Internet ✅[/]"))
        console.print("")  # Xuống dòng cho đẹp

        if latency < 200:
            status = f"[green]⚡ Mạng mượt ({int(latency)} ms)[/]"
        elif latency < 500:
            status = f"[yellow]🌀 Mạng trung bình ({int(latency)} ms)[/]"
        else:
            status = f"[red]🐢 Mạng chậm hoặc tắc ({int(latency)} ms)[/]"
        console.print(Align.center(status))

    except requests.ConnectionError:
        console.print("[bold red]❌ Không có kết nối Internet.[/]")
    except requests.Timeout:
        console.print("[bold red]⏱️ Kết nối quá thời gian chờ.[/]")
    except KeyboardInterrupt:
        console.print("\n[red]⛔ Bạn đã hủy kiểm tra mạng.[/red]")
        sys.exit(0)
...
if __name__ == "__main__":
    ...
    console = Console()
    print()
    table = Table(title="★★★ SMS Spam Tool ★★★", show_header=False, show_lines=True, expand=True)
    table.add_column("Thông tin", justify="center", style="cyan", no_wrap=True)
    table.add_row("[bold magenta]Tiến Kiệt[/bold magenta]")
    table.add_row("[blue]https://kie170120077-prog.github.io[/blue]")
